...
public class ArrayBufferPrototypeTest extends BaseTest {
    ...
    public void testGetByteLength() {
        ...
        result = ArrayBufferPrototype.getByteLength(ctx, nonBuffer, new JSValue[]{});
    ...
    public void testGetDetached() {
        ...
        JSArrayBuffer buffer = new JSArrayBuffer(16);
        JSValue result = ArrayBufferPrototype.getDetached(ctx, buffer, new JSValue[]{});
        assertTrue(result.isBooleanFalse());

        // Normal case: detached buffer
        buffer.detach();
        result = ArrayBufferPrototype.getDetached(ctx, buffer, new JSValue[]{});
        assertTrue(result.isBooleanTrue());

        // Edge case: called on non-ArrayBuffer
        JSValue nonBuffer = new JSString("not a buffer");
        result = ArrayBufferPrototype.getDetached(ctx, nonBuffer, new JSValue[]{});
        assertTypeError(result);
        assertPendingException(ctx);
    }
    ...
    public void testGetMaxByteLength() {
        ...
        // Edge case: called on non-ArrayBuffer
        JSValue nonBuffer = new JSString("not a buffer");
        result = ArrayBufferPrototype.getMaxByteLength(ctx, nonBuffer, new JSValue[]{});
        assertTypeError(result);
        assertPendingException(ctx);
    }
    ...
    public void testGetResizable() {
        // Normal case: non-resizable buffer
        JSArrayBuffer buffer = new JSArrayBuffer(16);
        JSValue result = ArrayBufferPrototype.getResizable(ctx, buffer, new JSValue[]{});
        assertTrue(result.isBooleanFalse());

        // Normal case: resizable buffer
        JSArrayBuffer resizableBuffer = new JSArrayBuffer(16, 64);
        result = ArrayBufferPrototype.getResizable(ctx, resizableBuffer, new JSValue[]{});
        assertTrue(result.isBooleanTrue());

        // Edge case: called on non-ArrayBuffer
        JSValue nonBuffer = new JSString("not a buffer");
        result = ArrayBufferPrototype.getResizable(ctx, nonBuffer, new JSValue[]{});
        assertTypeError(result);
        assertPendingException(ctx);
    }
    ...
    public void testResize() {
        ...
        JSValue result = ArrayBufferPrototype.resize(ctx, buffer, new JSValue[]{new JSNumber(32)});
        assertTrue(result.isUndefined());
        assertEquals(32, buffer.getByteLength());

        // Normal case: resize to smaller size
        result = ArrayBufferPrototype.resize(ctx, buffer, new JSValue[]{new JSNumber(8)});
        assertTrue(result.isUndefined());
        assertEquals(8, buffer.getByteLength());

        // Edge case: resize non-resizable buffer
        JSArrayBuffer fixedBuffer = new JSArrayBuffer(16);
        result = ArrayBufferPrototype.resize(ctx, fixedBuffer, new JSValue[]{new JSNumber(32)});
        assertRangeError(result);
        assertPendingException(ctx);

        // Edge case: resize detached buffer
        buffer.detach();
        result = ArrayBufferPrototype.resize(ctx, buffer, new JSValue[]{new JSNumber(16)});
        assertRangeError(result);
        assertPendingException(ctx);

        // Edge case: resize beyond maxByteLength
        JSArrayBuffer buffer2 = new JSArrayBuffer(16, 64);
        result = ArrayBufferPrototype.resize(ctx, buffer2, new JSValue[]{new JSNumber(128)});
        assertRangeError(result);
        assertPendingException(ctx);

        // Edge case: called on non-ArrayBuffer
        JSValue nonBuffer = new JSString("not a buffer");
        result = ArrayBufferPrototype.resize(ctx, nonBuffer, new JSValue[]{new JSNumber(16)});
        assertTypeError(result);
        assertPendingException(ctx);

        // Edge case: no arguments
        JSArrayBuffer buffer3 = new JSArrayBuffer(16, 64);
        result = ArrayBufferPrototype.resize(ctx, buffer3, new JSValue[]{});
        assertTypeError(result);
        assertPendingException(ctx);
    }
    ...
    public void testSlice() {
        ...
        // Edge case: called on non-ArrayBuffer
        JSValue nonBuffer = new JSString("not a buffer");
        result = ArrayBufferPrototype.slice(ctx, nonBuffer, new JSValue[]{});
        assertTypeError(result);
        assertPendingException(ctx);

        // Edge case: slice on detached buffer
        buffer.detach();
        result = ArrayBufferPrototype.slice(ctx, buffer, new JSValue[]{});
        assertTypeError(result);
        assertPendingException(ctx);
    }
    ...
    public void testTransfer() {
        ...
        result = ArrayBufferPrototype.transfer(ctx, detachedBuffer, new JSValue[]{});
        assertTypeError(result);
        assertPendingException(ctx);

        // Edge case: called on non-ArrayBuffer
        JSValue nonBuffer = new JSString("not a buffer");
        result = ArrayBufferPrototype.transfer(ctx, nonBuffer, new JSValue[]{});
        assertTypeError(result);
        assertPendingException(ctx);
    }
    ...
    public void testTransferToFixedLength() {
        ...
        JSArrayBuffer detachedBuffer = new JSArrayBuffer(16);
        detachedBuffer.detach();
        result = ArrayBufferPrototype.transferToFixedLength(ctx, detachedBuffer, new JSValue[]{});
        assertTypeError(result);
        assertPendingException(ctx);

        // Edge case: called on non-ArrayBuffer
        JSValue nonBuffer = new JSString("not a buffer");
        result = ArrayBufferPrototype.transferToFixedLength(ctx, nonBuffer, new JSValue[]{});
        assertTypeError(result);
        assertPendingException(ctx);
    }
}