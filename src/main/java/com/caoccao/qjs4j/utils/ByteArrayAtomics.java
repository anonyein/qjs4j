...
public final class ByteArrayAtomics {
    /**
     * The array-element handle for {@code byte[]}. Not a view handle: it addresses the array's own elements and needs
     * no heap {@code byte[]} alignment, so it works wherever {@code VarHandle} itself does. Like the view handles below
     * it is created inside the {@code static} block's {@code try}, so a runtime without it degrades to the locked path
    ...
    /**
     * Whether the JDK's byte-array view handles still offer atomic access modes. Computed after the handles are
     * created; if creating any of them failed, this is {@code false} and every operation routes through
     * {@link Fallback}.
     */
    ...
    /**
     * The byte-array view handles and the array-element handle. Deliberately not field initialisers: on a runtime whose
     * {@code MethodHandles.byteArrayViewVarHandle} or {@code MethodHandles.arrayElementVarHandle} is missing or throws
     * on the call itself — most Android ART ports, and some JDK builds — a field initialiser would fail the whole class
     * with an {@code ExceptionInInitializerError} before {@link #supportsAtomicAccessModes()} could probe anything.
     * Creating them inside a {@code try} that also degrades {@code LOCK_FREE} lets a {@code NoSuchMethodError}, an
     * {@code UnsupportedOperationException} or any other failure of creation fall back to the locked path instead of
     * taking the class down.
     */
